...
- Poison Item을 먹은 경우, 꼬리가 2칸 제거되어 길이가 1 감소합니다. 길이가 3 미만이 되면 즉시 게임 오버입니다.
...
현재 스테이지의 **미션 4가지 조건을 모두(AND) 달성**하면 스테이지 클리어가 발생합니다.

| 지표 | 설명 |
|---|---|
| **B** | 뱀이 한 번이라도 도달한 최대 길이 ≥ 목표 |
| **+** | Growth Item 수집 누계 ≥ 목표 |
| **-** | Poison Item 수집 누계 ≥ 목표 |
| **G** | Gate 통과 누계 ≥ 목표 |

점수판 우측의 `[ Mission ]` 섹션에서 각 목표값과 달성여부를 실시간으로 확인할 수 있습니다.
...
스폰 주기 : 15틱마다 1개 시도
수명       : 300틱
효과       : 꼬리 2칸 제거 → 길이 -1 / 길이 3 미만이면 즉시 게임 오버
동시 존재  : Growth + Poison 합산 최대 3개
```
...
**스테이지별 클리어 미션 (B: 최대 길이, +: Growth 수집, -: Poison 수집, G: Gate 통과 — 4가지 모두 달성 시 클리어)**
...
- Poison Item 수집 후 뱀 길이가 3 미만이 되는 경우
- **반대 방향 키 입력**
