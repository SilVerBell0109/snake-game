...
```
Common.h              ← 전체 공유 상수·방향·셀값·Point/Item/Mission 구조체
│
...
- Poison Item을 먹은 경우, 꼬리가 2칸 제거되어 길이가 1 감소합니다.
...
스테이지 클리어는 **해당 스테이지의 미션 4가지 조건을 모두 달성**함으로써 이루어집니다.

| 조건 | 설명 | 점수판 표기 |
|---|---|---|
| **B** — 최대 길이 | 이번 스테이지에서 달성한 최대 뱀 길이 (줄어들어도 기록은 유지) | `B: 현재/최대` |
| **+** — Growth 획득 수 | Growth Item(+1~+9)을 먹은 총 횟수 | `+: N` |
| **-** — Poison 획득 수 | Poison Item(--)을 먹은 총 횟수 | `-: N` |
| **G** — Gate 통과 수 | Gate를 통과한 총 횟수 | `G: N` |

**스테이지별 미션 목표:**

| 스테이지 | B (최대 길이) | + (Growth) | - (Poison) | G (Gate) |
|---|---|---|---|---|
| Stage 1 | ≥ 6 | ≥ 2 | ≥ 1 | ≥ 0 |
| Stage 2 | ≥ 7 | ≥ 3 | ≥ 1 | ≥ 1 |
| Stage 3 | ≥ 8 | ≥ 4 | ≥ 2 | ≥ 2 |
...
점수판 우측의 `[ Mission ]` 섹션에서 각 조건의 달성 여부를 `[v]`(달성) / `[ ]`(미달성)으로 실시간 확인할 수 있습니다. 4개가 모두 `[v]`가 되면 Stage Clear가 발생합니다.
...
currentScore -= (poisonCount - prevPoison) * 5;                    // Poison
...
**전략:**
- 개방된 공간에서 Growth Item을 수집하며 길이 늘리기
- Poison을 1회 이상 먹어야 미션 달성 (-≥1)
- Gate는 자동으로 카운트되므로 적극 활용
...
**전략:**
- 가로벽의 col 5 틈을 통해 상하 구역을 오갈 수 있음
- Gate 1회 이상 통과 필요 (G≥1) — 내부 Wall Gate 활용 유효
- 세로벽 오른쪽 좁은 공간에서 자기 몸통 주의
...
**전략:**
- 기둥 사이 좁은 통로에서 자기 몸통과 충돌하지 않도록 계획적인 이동 필요
- Ghost 아이템이 있으면 좁은 구역에서 유용
- Poison 2회 이상, Gate 2회 이상 필요 — 의도적으로 수행 필요
...
**전략:**
- 중앙 개방 구역(약 5×5칸)을 허브로 활용
- Gate 3회 통과 필요 → 적극적인 Gate 활용 필수
- 길이를 10 이상 키우려면 충분한 공간 확보와 자기 몸통 회피 계획이 중요
...
└── snake-game/
    ...
    ├── Common.h            ← 상수·방향·셀값·Point/Item/Mission 구조체
...
### 미션 정의 (Board.cpp — MISSIONS[4])

| 스테이지 | targetLength (B) | targetGrowth (+) | targetPoison (-) | targetGate (G) |
|---|---|---|---|---|
| Stage 1 | 6 | 2 | 1 | 0 |
| Stage 2 | 7 | 3 | 1 | 1 |
| Stage 3 | 8 | 4 | 2 | 2 |
