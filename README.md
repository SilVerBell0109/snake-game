...
- 우측: 실시간 점수판 — 스테이지·시간·점수·최고점수·뱀 길이·아이템 수·Growth 수집 현황·활성 효과 표시
...
│   ├── Food          ← Growth Item (+1~+9) 스폰·수명·소비·수집 추적·전체 수집 판정
...
`Snake::move()`는 이동·충돌·아이템 소비를 단일 함수에서 처리하며, 필요한 객체를 모두 참조로 수신합니다. 이를 통해 각 클래스가 자신의 상태만 책임지는 구조를 유지합니다.
...
```
[2][2][2][2][2][2][2][2][2][1][1][1][2][2][2][2][2][2][2][2][2]  ← 상단
[2]                                                          [2]
...          (내부: 스테이지에 따라 Wall/빈공간)              ...
[2]                                                          [2]
[2][2][2][2][2][2][2][2][2][1][1][1][2][2][2][2][2][2][2][2][2]  ← 하단
 ↑                           ↑↑↑                           ↑
...
- 내부 Wall(스테이지별 장애물)도 `Wall (1)` → Gate 생성 가능
...
> **반대 방향 입력은 무시됩니다.** 예를 들어 오른쪽으로 이동 중에 왼쪽 키를 눌러도 게임 오버가 발생하지 않으며, 현재 방향을 유지합니다. 이전 Snake 게임 규칙과 다르게 변경된 사항입니다.
...
`Snake::setNextDir()`에 저장된 방향은 **다음 틱 이동 시작 시** 적용됩니다. 현재 방향의 반대 방향이 입력되면 버퍼에 저장하지 않고 무시합니다.
...
스테이지 클리어는 **Growth Item +1부터 +9까지 9개를 모두 수집**함으로써 달성합니다.

```
수집 현황: [ +1(초록) +2(초록) +3(빨강) +4(빨강) ... ] → 초록=수집완료, 빨강=미수집
9개 모두 초록으로 바뀌면 → Stage Clear! (+200pts)
```

Growth Item은 한 번에 최대 3개까지 맵에 존재하며, 15틱마다 한 번씩 새로운 번호가 스폰을 시도합니다. 이미 맵에 올라간 번호는 중복 스폰되지 않으며, 수집 완료된 번호도 다시 스폰되지 않습니다. 수명(300틱) 안에 먹지 못하면 사라지지만, 이후 다시 스폰될 수 있어 결국 9개 모두 수집 가능합니다.
...
```
스폰 주기 : 15틱마다 1개 시도 (맵에 최대 3개 동시 존재)
수명       : 300틱 (약 60초 @ 200ms/틱)
효과       : 뱀 길이 +1, 해당 번호를 수집 완료로 표시
번호 범위  : +1 ~ +9 (스테이지 클리어에 필요한 9개)
```
...
진출 위치가 Wall·Immune Wall·다른 Gate라면 다음 우선순위로 넘어갑니다. 모든 방향이 막혀 있으면 fallback으로 반대 방향을 반환하며, 이후 Snake가 벽으로 처리합니다.
...
```
  ...
  [WALL]  [GATE B]  [WALL]
             ↓
  → 진출 방향: 아래(직진) 시도 → 벽이므로 왼쪽(시계90°) 시도 → 성공
```
...
**Speed/Slow 상호 배제:**  
Speed가 활성 효과로 남아 있으면 Slow가 맵에 스폰되지 않고, 반대도 마찬가지입니다. 두 아이템이 맵에 동시에 존재하는 것도 방지됩니다.
...
- 초보자 난이도, 기본 조작과 아이템 수집 학습에 적합

**전략:**
- 개방된 공간에서 Growth Item을 찾아 이동
- Gate가 테두리에 생성되므로 Gate 통과 연습 가능
- Poison과 자기 몸통만 주의하면 됨
...
**특징:**
- **가로벽:** row 6, col 2-4 (3칸) + col 6-8 (3칸) — 중앙 col 5에 틈이 있어 통과 가능
- **세로벽:** col 14, row 12-17 (6칸) — 맵 오른쪽 하단을 좌우로 분리
- 두 벽이 맵을 비대칭으로 분할하여 이동 경로가 제한됨
- Gate가 내부 Wall에도 생성될 수 있어 예기치 않은 순간이동 발생 가능

**전략:**
- 가로벽의 col 5 틈을 통해 상하 구역을 오갈 수 있음
- 세로벽 오른쪽(col 15-19)은 좁은 공간, 자기 몸통 충돌 주의
- 내부 Wall Gate 활용 시 먼 거리를 단번에 이동 가능
...
**특징:**
- 각 구석에 3×3 크기의 Wall 기둥이 배치됨
- 4개의 기둥이 맵 내부를 **5개의 구역**(중앙, 상좌, 상우, 하좌, 하우)으로 분할
- 기둥 사이의 통로 폭이 2~4칸으로 좁아 긴 뱀은 방향 전환에 제약이 생김
- 기둥 Wall에도 Gate 생성 가능

**전략:**
- 기둥 사이 좁은 통로에서 자기 몸통과 충돌하지 않도록 계획적인 이동 필요
- 뱀이 길어질수록 U자 이동 시 공간 계산이 중요해짐
- Ghost 아이템이 있으면 좁은 구역에서 유용하게 활용 가능
...
##  ##  ##  ##  ##  ##  ##  ##  ##  ██  ██  ██  ##  ##  ##  ##  ##  ##  ##  ##  ##
  ...
  세로벽: col 10, row 3-7 및 row 13-17 (중앙 row 8-12 는 개방)
  가로벽: row 10, col 3-7 및 col 14-18 (중앙 col 8-13 은 개방)
```

**특징:**
- 십자형 벽이 맵을 **9개의 구역**으로 나눔 (3×3 격자)
- 각 벽의 중앙 부분은 열려 있어 구역 간 이동이 완전히 차단되지는 않음
- 가장 복잡한 장애물 배치로 긴 뱀의 이동 경로가 크게 제한됨
- 벽이 가장 많아 Gate 생성 위치가 다양하고, 예상치 못한 순간이동 빈도가 높음

**전략:**
- 중앙 개방 구역(약 5×5칸)을 허브로 활용하여 각 구역 간 이동
- 좁은 구역에서 Growth Item 수집 시 자기 몸통 충돌 위험이 높아 Shield·Ghost 아이템이 중요
- Gate를 적극 활용하면 멀리 떨어진 Growth Item까지 빠르게 도달 가능
...
`Makefile`이 `uname`으로 OS를 자동 감지하여 ncurses 헤더/라이브러리 경로를 분기합니다. macOS에서는 Homebrew 설치 경로(`/opt/homebrew`)를, Linux에서는 시스템 기본 경로를 사용합니다.
...
g++ -std=c++11 -Wall -Wextra \
    ...
    -c main.cpp Board.cpp Snake.cpp Item.cpp Food.cpp Poison.cpp Gate.cpp Special.cpp

g++ ... -o snake \
    -L/opt/homebrew/opt/ncurses/lib -lncursesw
```
...
├── Makefile                ← (루트) 단일 파일용 구 버전 Makefile
└── snake-game/
    ├── Makefile            ← OS 자동 감지(Darwin/Linux), 멀티파일 빌드
    ├── Common.h            ← 상수·방향·셀값·Point/Item/Mission 구조체·oppositeDir()
    ├── Board.h / Board.cpp ← 맵 배열·label 배열·ncurses 2윈도우·14색상쌍 렌더링
    ├── Item.h  / Item.cpp  ← ItemBase 추상 클래스 (spawnItem/updateItems 공통 헬퍼)
    ├── Food.h  / Food.cpp  ← Growth Item: 번호 할당·수집 추적·전체 수집 클리어 판정
...
| `ITEM_LIMIT` | 3 | Growth/Poison 동시 최대 개수 |
